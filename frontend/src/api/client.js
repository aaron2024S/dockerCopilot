...
// 自动更新API
// 配置返回的是扁平对象：{ enabled, deleteOldImage, protectSelf, excludeList,
// lastRunAt, lastTrigger, lastResult, configPath, running }
export const autoUpdateAPI = {
  getSetting: () => apiClient.get('/api/autoUpdate'),

  // PUT 走 JSON body。后端三个开关是值类型（非指针），所以每次必须全量回传，
  // 否则未传的开关会被当成 false 写进去；excludeList 传 undefined 时后端保持原值。
  updateSetting: (payload) => apiClient.put('/api/autoUpdate', payload),

  // 立即执行一轮：后端异步返回，需轮询 getSetting 的 running / lastResult 看结果
  run: () => apiClient.post('/api/autoUpdate/run'),

  // 各容器本轮是否会被自动更新，含被跳过的原因
  getCandidates: () => apiClient.get('/api/autoUpdate/candidates'),
}

// 手动检测镜像更新
//
// 一轮检测要逐个 HEAD 各镜像仓库，耗时远超默认的 10s 超时，所以 run 单独放宽到 3 分钟。
// 但这个上限只是「前端愿意等多久」，不是检测本身的时限：服务端有它自己的节奏，
// 客户端断开后仍会继续跑完并把结论写进内存。
// 所以超时后不要当失败——改去轮询 status，等 running 变 false 且 lastCheckedAt 发生变化，
// 就能拿到真实结果；顺带这个接口也能用来判断「是不是已经有一轮在跑了」。
export const CHECK_UPDATE_TIMEOUT = 180000

export const checkUpdateAPI = {
  run: () => apiClient.post('/api/checkUpdate', null, { timeout: CHECK_UPDATE_TIMEOUT }),

  // 只读内存快照，服务端零开销，可以放心高频轮询
  status: () => apiClient.get('/api/checkUpdate/status', { timeout: 10000 }),
}
