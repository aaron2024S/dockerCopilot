...
/**
 * 统一版本号展示格式。
 * 后端返回的可能是 "v2.1.3" 也可能是 "2.1.3"，这里统一补上 v，避免出现 "v v2.1.3"。
 * 读不到版本时返回 null，由调用方决定显示什么（不要写死一个假的版本号）。
 */
...
export function Sidebar({ activeTab, onTabChange, onLogout, isCollapsed = false, onToggleCollapse, windowWidth = 1024 }) {
  ...
  // 版本号的三种状态：拿到了真实版本 / 还在读 / 读不到
  const versionLabel = formatVersion(backendVersion)
  const versionPlaceholder = versionLoading ? '···' : '未知'
        ...
        <div className="fixed top-0 left-0 right-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-3 sm:px-4 z-40 shadow-sm" style={{ paddingTop: 'env(safe-area-inset-top)', paddingBottom: '0.875rem', height: 'calc(3.5rem + env(safe-area-inset-top))' }}>
          ...
          >
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="px-1.5 py-[2px] text-[10px] font-semibold leading-none rounded-md bg-primary-100 text-primary-700 ring-1 ring-primary-200 dark:bg-primary-500/20 dark:text-primary-300 dark:ring-primary-500/30">
                增强版
              </span>
              ...
              >
                ...
                {!isCollapsed && isMobileSize === false && (
                  <div className="text-left transition-all duration-300 min-w-0 flex-1">
                    {/* 角标用绝对定位贴在标题右上角：不占布局宽度，窄侧边栏下也不会被挤到第二行 */}
                    <h1 className="relative inline-block whitespace-nowrap text-base sm:text-lg font-bold text-gray-900 dark:text-white">
                      Docker Copilot
                      <span className="absolute -top-3 right-0 translate-x-1/4 px-1.5 py-[2px] text-[10px] font-semibold leading-none whitespace-nowrap rounded-md bg-primary-100 text-primary-700 ring-1 ring-primary-200 shadow-sm dark:bg-primary-500/20 dark:text-primary-300 dark:ring-primary-500/30">
                        增强版
                      </span>
                    </h1>
            ...
            {isCollapsed ? (
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-center">
                  <span className="w-3 h-3 rounded-full bg-emerald-500 shadow-sm shadow-emerald-400/50 dark:shadow-emerald-600/50" title={`运行中 - ${versionLabel || versionPlaceholder}`} />
                </div>
                    ...
                    {versionLabel ? (
                      ...
                      <span
                        className={cn(
                          ...
                          versionLoading ? "text-gray-300 dark:text-gray-600" : "text-gray-400 dark:text-gray-500"
                        )}
                        title={versionLoading ? '正在读取后端版本…' : '未能读取到后端版本'}
                      >
                        {versionPlaceholder}
                      </span>
                    )}
