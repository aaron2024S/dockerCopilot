...
/**
 * 上次成功读到的后端版本号，存在 localStorage。
 * 接口回来之前先用它顶着，免得页面先闪一个写死的假版本号再跳成真实版本。
 */
const VERSION_CACHE_KEY = 'dockerCopilot:lastBackendVersion'

function readCachedVersion() {
  try {
    const value = window.localStorage.getItem(VERSION_CACHE_KEY)
...
export function useVersionCheck() {
  ...
  const [cachedVersion, setCachedVersion] = useState(readCachedVersion)

  // 查询后端版本信息
  const { data: versionData, refetch, isPending } = useQuery({
    queryKey: ['version'],
    queryFn: async () => {
      try {
        // 获取本地版本信息
  ...
  // 拿到真实版本后缓存起来，下次进页面直接显示它
  const resolvedVersion = versionData?.backendVersion

  useEffect(() => {
    if (!resolvedVersion || resolvedVersion === 'unknown') return
    setCachedVersion(resolvedVersion)
    try {
      window.localStorage.setItem(VERSION_CACHE_KEY, resolvedVersion)
    } catch (error) {
  ...
  return {
    ...
    // 版本数据。接口没回来时用上次缓存顶着，仍然是空才说明真的读不到。
    backendVersion: resolvedVersion && resolvedVersion !== 'unknown' ? resolvedVersion : cachedVersion,
    versionLoading: isPending && !cachedVersion,
